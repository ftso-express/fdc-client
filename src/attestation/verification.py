...
def valid_lut(lut: int, lut_limit: int, round_start: int) -> bool:
    ...
    """
    if lut > round_start:
        # If lut is in the future, the difference will be very large due to
        # unsigned integer underflow, which is correctly handled by Python's
        # arbitrary-precision integers.
        return False

    lhs = round_start - lut
    return lhs < lut_limit
...
def get_lut(response: Response) -> int:
    ...
    return int.from_bytes(lut_bytes, 'big')
